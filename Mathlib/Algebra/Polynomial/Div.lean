...
theorem degree_modByMonic_le_left (p q : R[X]) :
    degree (p %ₘ q) ≤ degree p := by
  nontriviality R
  by_cases hq : q.Monic
...
theorem natDegree_modByMonic_le_left (p q : R[X]) :
    natDegree (p %ₘ q) ≤ natDegree p :=
  natDegree_le_natDegree (degree_modByMonic_le_left p q)
